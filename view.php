...
<p style="padding-left: 100px">Name : <?php echo $userdata['name']  ?><br>Roll No : <?php echo $userdata['rollno']  ?><br>Branch : <?php echo $userdata['branch']  ?><br>Semester : <?php echo $userdata['semester']  ?></p>
...
<table class="table table-striped" style="font-size: 17px;">
	...
	while ($row = $iquery->fetch_assoc()) {
		...
		<td>".$data['credit']."</td>
		<td>".$data['slot']."</td>
		<td><a href='http://localhost/elective_management/accept.php?cid=".$data['code']."&sid=".$rollno."'><button class='btn btn-success btn-sm'>Accept</button></a><a href='http://localhost/elective_management/reject.php?cid=".$data['code']."&sid=".$rollno."'><button class='btn btn-danger btn-sm'>Reject</button></a></td>
		<td>".$value."</td>
	</tr>";
	$i++;
	}
...
<a href="fahome.php"><button class="btn btn-info btn-sm">Back</button></a>
</div>
</body>
</html>